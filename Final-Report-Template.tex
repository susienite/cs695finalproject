...
3) k-Nearest Neighbors : is a machine leawrning algorithm that is used for classification and regression tasks. It is an algorithm that does not make any assumptions about the underlying data distribution. This algorithm is useful for this data because for the samples that are on the board of benign and malignant. It will compare to the in order to make a prediction. It will utilize the distance metric and then use the labels of those neighbors to determine the label of the new data point. 
4) Logistic Regression : This algorithm aims to find if a diagnosis is benign or malignant based on the raidus given. So depending on that input value it will either be benign or malignant.
...
\begin{enumerate}
    ...
    \item Logistic Regression : this is a machine learning algorithm which uses binary classification tasks. In this particular instance it is predicting the probability if a diagnose is benign or malignant. This is able to happen by the use of Logistic function to the training data. which will map the input features. Which are the columns 'diagnosis'. The activation function was the sigmoid function which maped any real value between 0 and 1. The use of the "X_train" and "y_train" was for the training data and labels. 'X_test" represented the test data. What was also used was the '.fit' method which was used to train the logistic regression model, and 'predict' method was used to predict was used to make predictions on the new data. 
